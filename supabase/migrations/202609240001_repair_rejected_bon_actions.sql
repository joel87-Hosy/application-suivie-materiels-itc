-- Reinstall correction RPC and related workflow even when migration 202609230005 was missed.
BEGIN;
...
CREATE OR REPLACE FUNCTION public.delete_rejected_stock_request(request_key text, expected_decision jsonb)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path=public AS $$
DECLARE actor public.app_profiles; request public.app_records;
BEGIN
 SELECT * INTO actor FROM current_app_profile();
 IF actor.role IS DISTINCT FROM 'Gestionnaire' THEN RAISE EXCEPTION 'Suppression réservée au gestionnaire concerné.'; END IF;
 SELECT * INTO request FROM app_records WHERE collection='demandes' AND record_key=request_key AND company_id=actor.company_id FOR UPDATE;
 IF request.record_key IS NULL OR request.payload->>'assignedGestionnaireUid' IS DISTINCT FROM actor.user_id::text THEN RAISE EXCEPTION 'Bon absent ou affecté à un autre gestionnaire.'; END IF;
 IF request.payload->>'status' IS DISTINCT FROM 'REFUSEE VALIDATEUR' OR request.payload->'validatorDecision' IS DISTINCT FROM expected_decision THEN RAISE EXCEPTION 'Bon déjà modifié. Actualisez la liste.'; END IF;
 INSERT INTO app_records(collection,record_key,company_id,payload) VALUES('platformAuditLogs',gen_random_uuid()::text,actor.company_id,
   jsonb_build_object('action','SUPPRESSION_BON_REFUSE','requestId',request.payload->>'id','requestKey',request_key,'company_id',actor.company_id,'by',actor.user_id,'name',actor.profile->>'name','at',now(),'before',request.payload));
 DELETE FROM app_records WHERE collection='demandes' AND record_key=request_key AND company_id=actor.company_id;
END $$;
REVOKE ALL ON FUNCTION public.delete_rejected_stock_request(text,jsonb) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.delete_rejected_stock_request(text,jsonb) TO authenticated;
