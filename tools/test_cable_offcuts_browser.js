...
const core=require('../functions/cable-offcuts-core');
let server,chrome,state={},counter=0;const sockets=[];
const actors=Object.fromEntries(['Technicien','Coordinateur','Gestionnaire','Contrôleur','Superviseur'].map(role=>[role,{uid:role,name:role,role,company_id:'A',is_active:true,controlScopes:{MOOV:true}}]));
...
async function main(){
 ...
 await coord.refresh();await coord.action('approveRequest');await manager.refresh();await manager.action('issue');
 ...
 for(const role of ['Contrôleur','Superviseur']){const viewer=await page(port,role);assert.equal(await viewer.evaluate('document.querySelectorAll("form, [data-action=receiveReturn], [data-action=issue]").length'),0);assert.match(await viewer.evaluate('document.body.textContent'),/BON-ORIGINE/);}
 assert.equal(counter,8);
 console.log('PASS: real browser technician return → coordinator → manager receipt, manual entry, reuse request → validation → issue, PDF button, second return and read-only supervision.');
}
