...
(async()=>{
 ...
 const context={appData:{derniereDateScan:'yesterday',scansDuJour:[{id:'OLD'}],demandes:[{id:'BON',demandeurName:'Équipe',status:'EN ATTENTE VALIDATEUR'}]},save:async()=>{saves++;return true},document:{getElementById:()=>null},navigator:{},setTimeout:()=>{},console};
 vm.createContext(context);vm.runInContext(extract('verifierReinitialisationQuotidienne')+'\n'+extract('handleScanSuccess'),context);
