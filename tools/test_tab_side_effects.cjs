...
(async()=>{
 ...
 const scanned=[];context.window.BonScanner={scan:async id=>scanned.push(id)};
 await context.handleScanSuccess('BON');assert.deepEqual(scanned,['BON']);assert.equal(saves,0,'scanner delegates server verification instead of saving a stale snapshot');
 ...
 await context.markNotificationsAsRead();assert.equal(context.appData.notifications[0].lu,true);assert.equal(context.appData.notifications[1].lu,false);assert.equal(saves,0,'notification read never invokes general save');assert.equal(updates,1);
 console.log('PASS: scanner delegates authoritative verification without general save, navigation has no writes, targeted notification UI update.');
